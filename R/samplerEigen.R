...
Eigen.sampler.bingham <- function(X,
    ...
    for(i in 2:n.mcmc){
        ...
        mcmc.lambda[i, ] <- sampleLambda(lambda_hat, n, init_params_Lambda)

        #sample prior
        lambda.ind <- findInterval(log(mcmc.lambda[i,]), init_params_Lambda$a.vec, left.open = T)
...
Eigen.sampler.old <- function(X,
    ...
    res.E <- oracle.metrop.sampler(E$vectors,X, E$values, mcmc.samples)
