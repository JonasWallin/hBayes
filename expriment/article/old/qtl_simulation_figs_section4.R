...
for(n in ns){
    ...
    stopCluster(cl)
    save( betas.lasso,intercept.lasso,
          betas.SSlasso, intercept.sslasso,
          betas.ridge, intercept.ridge,
          beta.npbayes,beta.forwardback,intercept.forwardback,
          beta.ash,intercept.ash, file = "simulation2.RData")
}
...
if(plot.fig){
    ...
    res.np <- plot.graph(beta.npbayes[[q]],window.size, markers, beta_true = beta_ ,name="hBayes")
    fig.np <- res.np$fig
    ...
    res <- gibbs.normal(n.gibbs,
                        ...
                        beta.npbayes[[i]])
    #print cdf
    pi.cdf      <- rbind(0,apply(exp(res$pi.gibbs[floor(n.gibbs/3):n.gibbs,]),1,cumsum))
    ...
    for(i in 1:1000){
        ...
        beta_ <- mu[1] + tau * (shift * (V-1) +  sqrt(V)*Z)
        beta_[qtl.pos] <- beta.true
        beta_quant <- c(beta_quant, beta_)
    }
    pi.cdf.true <- rep(0, length(res$a.vec))
    for(j in 1:length(res$a.vec)){
        pi.cdf.true[j] <- mean(beta_quant <= res$a.vec[j])
    }
    if(save.fig)
        pdf('CDF_sim.pdf')
    par(mfrow=c(1,1))
    plot(res$a.vec,pi.quant[2,],col = 1,lwd = 2,xlim=c(-0.1,0.05), main = "",ylab = "CDF",xlab = expression(beta), type='l',
         cex.lab = 1.5)
    lines(res$a.vec,pi.quant[1,],col=1,lty = 2,lwd = 2)
    lines(res$a.vec,pi.quant[3,],col=1,lty = 2,lwd = 2)

    lines(res$a.vec, pi.cdf.true, col=2, lwd=2)
    ...
    MixGeneObj <- mixedModelForwardBackward(MixGeneObj,
                                            markers,
                                            dupl  = findDuplicate(Xs[[i]]))
    alpha = (length(MixGeneObj$beta)-2)/dim(Xs[[i]])[2]
    dens_norm <- (1- alpha) *pnorm(res$a.vec,mean=MixGeneObj$beta[2], sd = MixGeneObj$tau)
    if((length(MixGeneObj$beta)-2) > 0){
    for(i in 1:(length(MixGeneObj$beta)-2))
        dens_norm <- dens_norm + alpha * (res$a.vec > MixGeneObj$beta[i+2])
    }
    lines(res$a.vec,dens_norm,col=3,lwd=2)

    CDF_ridge <- ecdf(betas.ridge_unadj[[q]])
    lines(res$a.vec,CDF_ridge(res$a.vec),col=4,lwd=2)
    CDF_lasso <- ecdf(betas.lasso_unadj[[q]])
    lines(res$a.vec,CDF_lasso(res$a.vec),col=6,lwd=2)
