...
#fig.np <- res.np$fig
#fig.np <- fig.np +  labs(title=paste("(window size = ", window.size,")",sep=""))
...
CI_int = c(0.25,0.75)
res.np <- plot.graph2(Betas.np, markers, beta_true = NULL, x.sim =res$beta.gibbs[burnin:n.gibbs,] ,probs=CI_int)
fig.np <- res.np$fig
#fig.np <- fig.np +  labs(title=paste("(window size = ", window.size,")",sep=""))

#fig.np <- fig.np+ theme(plot.title = element_text(hjust = 0.5))
fig.np <- fig.np + ylab(TeX("$\\beta$"))+ theme(axis.title.y = element_text(angle=0,vjust=0.5,size=14,face="bold"))
fig.np <- fig.np + theme_bw()+ theme(axis.line = element_line(colour = "black"),
                         panel.border = element_blank(),
                         panel.background = element_blank()) + ylim(c(-0.3,0.2))
fig.np
ggsave('np_real.jpeg',fig.np)
...
# plot traceplots
##
interv = n.gibbs:burnin

plot(interv, res$beta.gibbs[interv ,25],type='l',ylab='iter',xlab=expression(beta), ylim=c(-0.3,0.3))
lines(interv, res$beta.gibbs[interv ,24],type='l',ylab='iter',xlab=expression(beta), ylim=c(-0.3,0.3), col='red')
#lines(interv, res$beta.gibbs[interv ,it-2],type='l',ylab='iter',xlab=expression(beta[it-1]), ylim=c(-0.3,0.3),col='blue')
#plot(interv, res$beta.gibbs[interv ,49],type='l',ylab='iter',xlab=expression(beta[49]), ylim=c(-0.3,0.3))
...
fig.np2 <- fig.np
fig.np2 <- fig.np2  + geom_ribbon(data = data.frame(locus=1:dim(X)[2],
                                                      low = qnorm(0.05, mean=mu, sd=tau),
                                                      upp = qnorm(0.95, mean=mu, sd=tau)),
...
fig.np2 <- fig.np2  + geom_ribbon(data = data.frame(locus=1:dim(X)[2],
                                                    low = qnorm(0.025, mean=beta_forward, sd= MixGeneObj_forward$tau),
                                                    upp = qnorm(0.975, mean=beta_forward, sd=MixGeneObj_forward$tau)),
                                  aes(x=locus,ymin = low, ymax=upp), alpha=0.1, fill='green')
...
Sigma <- solve(Q)
fig.np3 <- fig.np
fig.np3 <- fig.np3 +   geom_ribbon(data = data.frame(locus=1:dim(X)[2],
                                                      low = qnorm(CI_int[1], mean=Ebeta, sd=sqrt(diag(Sigma))),
                                                      upp = qnorm(CI_int[2], mean=Ebeta, sd=sqrt(diag(Sigma)))),
                                    aes(x=locus,ymin = low, ymax=upp), alpha=0.1, fill='red')
Q_2 = (t(X)%*%X)/MixGeneObj_forward$sigma^2
diag(Q_2) <- diag(Q_2)  + 1/MixGeneObj_forward$tau^2
Sigma_2 <- solve(Q_2)
Ebeta_2 = solve(Q_2, t(X)%*%(y - MixGeneObj_forward$beta[1] - X[,MixGeneObj_forward$find]%*%MixGeneObj_forward$beta[-c(1:2)])/MixGeneObj_forward$sigma^2 + MixGeneObj_forward$beta[2]/ MixGeneObj_forward$tau^2 )
Ebeta_2[MixGeneObj_forward$find] =Ebeta_2[MixGeneObj_forward$find] +MixGeneObj_forward$beta[-c(1:2)]
fig.np3 <- fig.np3 +   geom_ribbon(data = data.frame(locus=1:dim(X)[2],
                                                     low = qnorm(0.025, mean=Ebeta_2, sd=sqrt(diag(Sigma_2))),
                                                     upp = qnorm(0.975, mean=Ebeta_2, sd=sqrt(diag(Sigma_2)))),
                                   aes(x=locus,ymin = low, ymax=upp), alpha=0.1, fill='green')

# QTL sample
save.image('RealData.RData')
